import type { AvailableFood, AvailableFoodCategory, QualityCriteria } from './types'
import { getFoodItem, type FoodItem } from './foodDatabase'

export const foodCategoryLabels: Record<AvailableFoodCategory, string> = {
  protein: 'Білок', carbs: 'Вуглеводи', vegetables: 'Овочі', fruits: 'Фрукти', fats: 'Жири', dairy: 'Молочні', mixed: 'Змішані', other: 'Інше',
}
...
export function resolveAvailableFood(food: AvailableFood): FoodItem | undefined {
  return getFoodItem(food.foodId ?? legacyFoodIds[food.canonical] ?? '')
}

const hasProtein = (foods: AvailableFood[]) => foods.some(food => ['protein', 'dairy', 'mixed'].includes(resolveAvailableFood(food)?.category ?? food.category))
const hasCategory = (foods: AvailableFood[], category: AvailableFoodCategory) => foods.some(food => (resolveAvailableFood(food)?.category ?? food.category) === category)

export function pantryMissingQualityKeys(foods: AvailableFood[]): Array<keyof QualityCriteria> {
  if (!foods.length) return []
  const missing: Array<keyof QualityCriteria> = []
  if (!hasProtein(foods)) missing.push('proteinTargetReached')
  if (!hasCategory(foods, 'vegetables')) missing.push('vegetablesIncluded', 'fiberEnough')
  if (!hasCategory(foods, 'fruits')) missing.push('fruitsIncluded')
  if (!hasCategory(foods, 'fats')) missing.push('healthyFatsIncluded')
  if (new Set(foods.map(food => food.foodId ?? food.canonical)).size < 6) missing.push('varietyGood')
  return [...new Set(missing)]
}

export function applyPantryQuality(criteria: QualityCriteria, foods: AvailableFood[]): QualityCriteria {
  if (!foods.length) return criteria
  const result = { ...criteria }
  pantryMissingQualityKeys(foods).forEach(key => { result[key] = false })
  if (hasProtein(foods)) result.proteinTargetReached = true
  if (hasCategory(foods, 'vegetables')) result.vegetablesIncluded = true
  if (hasCategory(foods, 'fruits')) result.fruitsIncluded = true
  if (hasCategory(foods, 'fats')) result.healthyFatsIncluded = true
  result.fiberEnough = hasCategory(foods, 'vegetables') && (hasCategory(foods, 'fruits') || foods.some(food => ['oats_dry', 'buckwheat_dry', 'lentils_cooked'].includes(resolveAvailableFood(food)?.id ?? '')))
